...
//! earliest release whose `management_interface.proto` is wire-compatible with
//! this subset (field-identical from 2025.14 through the current 2026.x line).
...
/// Generated client and messages for the 2025.14-line management interface.
#[allow(
    missing_docs,
    unsafe_code,
    clippy::all,
...
mod proto {
    include!(concat!(
        env!("OUT_DIR"),
        "/v2025_14/mullvad_daemon.management_interface.rs"
    ));
}

use proto::management_service_client::ManagementServiceClient;

/// Mullvad management client speaking the 2025.14-line schema.
pub(crate) struct Backend {
    client: ManagementServiceClient<Channel>,
}

impl Backend {
    /// Wraps an already-connected gRPC channel in a 2025.14-line backend.
...
impl MullvadBackend for Backend {
    ...
    async fn disconnect(&self) -> Result<(), Error> {
        let mut client = self.client.clone();
        client
            .disconnect_tunnel(proto::StringValue {
                value: String::new(),
            })
            .await?;
        Ok(())
