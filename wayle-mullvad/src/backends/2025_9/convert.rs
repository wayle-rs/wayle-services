//! Conversions from the 2025.9-line wire types into the crate's public types.
//!
...
/// Converts a wire [`proto::TunnelState`] into a [`ConnectionStatus`].
...
/// its `location` — keeping the user's tunnel type, provider/ownership and
/// WireGuard/OpenVPN constraints intact. A custom or absent current setting
/// falls back to a fresh normal setting with default constraints.
...
mod tests {
    ...
    fn relay_settings_preserves_existing_constraints() {
        // The user's tunnel type, providers/ownership and the OpenVPN-era
        // constraints must all survive a connect that only changes the location.
        // (OpenVPN still exists on this daemon line, so dropping openvpn /
        // tunnel_type would silently reset the user's tunnel configuration.)
        let current = proto::RelaySettings {
            endpoint: Some(proto::relay_settings::Endpoint::Normal(
                proto::NormalRelaySettings {
                    ...
                    tunnel_type: proto::TunnelType::Wireguard as i32,
        ...
        assert_eq!(normal.tunnel_type, proto::TunnelType::Wireguard as i32);
